...
export async function GET() {
  ...
  // Studies carry a lastmod from their publish date; everything else is
  // a plain location as before.
  const lastmod = new Map(
    studiesLive() ? STUDIES.filter((s) => s.publishedAt).map((s) => [`${base}/studies/${s.slug}`, s.publishedAt]) : []
  );

  const xml = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
${urls.map((u) => `  <url><loc>${u}</loc>${lastmod.has(u) ? `<lastmod>${lastmod.get(u)}</lastmod>` : ''}</url>`).join('\n')}
</urlset>
`;
